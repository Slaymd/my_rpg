...
void reinit_var(rpg_t *rpg, entity_t *ent, map_t *map)
{
	...
	ent->hp > 0 ? disp_rect_at(rpg->wd, map, ent->rect, ent->pos) : 0;
	...
	ent->hp > 0 ? disp_rect_at(rpg->wd, map, ent->hp_bar, ent->pos) : 0;
}
...
void display_ennemy(rpg_t *rpg, entity_t *ent, map_t *map, int count)
{
	if (count == 5 && ent->mirror == 0) {
		...
		if (ent->mirror == 2 && count == 5) {
			ent->square.left = ent->square.left +
			ent->movement >= 2500 ? 2500 : ent->square.left + 177;
			sfSprite_setTextureRect(ent->sprite, ent->square);
		}
	}
